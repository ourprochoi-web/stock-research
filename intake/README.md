...
5. **자동 수집** — `collect.py`가 매일 워크플로에서 일일 브리핑(soonsal)과 TSMC 월매출 페이지를 받아 `collected.jsonl`에 한 줄과 `files/`에 스냅샷을 남긴다. 판단은 하지 않는다. 라우팅은 다음 세션이 `status:ok · routed:false` 항목을 읽어서 한다.
