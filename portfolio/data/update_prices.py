...
WATCH = {
    # 2026-09-09 추가 — 사용자 관심종목 「보유」 탭 스크린샷에 있는데 추적 밖이던 4종목.
    # RF머트리얼즈는 아카이브 언급이 0편인데 그날 +22.61%로 마감했고, 그 판단을
    # 물었을 때 시세도 재무도 손으로 받아야 했다.
    # 🔴 2026-09-10 정정(C3급) — 그때 「포지션이 있는데 시세도 판단도 없다」고 적었으나
    #    사용자가 "RF머트리얼즈는 실제 포지션이 아니다"라고 확인했다. 탭 이름이 「보유」였을 뿐이며
    #    보유로 읽은 것은 내 추론이고 반증됐다. 추적 자체는 유지한다 — 관심종목이고,
    #    그날 물었을 때 시세가 없어서 손으로 받아야 했던 것은 사실이기 때문이다.
    "RF머트리얼즈": "327260",
    "티에스이": "131290",
    "샘씨엔에스": "252990",
    "서울바이오시스": "092190",
    "한화에어로스페이스": "012450",
    "삼성전기": "009150",
    # 삼화콘덴서: 무라타 범용 MLCC 품번 단계적 종료(Digitimes 2026-09-10)로
    # 「범용을 계속 만드는 쪽」 축이 열렸는데 판단 0편·시세 밖이었다.
    # 판단은 세우지 않고 시세만 받는다(§B4) — 2026-09-11 장중 +20.31%.
    "삼화콘덴서": "001820",
    # 2026-09-14 국내 보안 — 삼프로 테마(젠슨 황 발언). 판단은 세우지 않고 시세만(§B4).
    "엑스게이트": "356680", "드림시큐리티": "203650", "안랩": "053800", "파이오링크": "170790",
    # 아모텍: 2026-09-11 상한가(+29.98%). 무라타 단종 축의 세 번째 이름인데
    # 범용이 아니라 광통신용 MLCC(②·공시 미대조)다. 판단은 세우지 않고 시세만 받는다.
    "아모텍": "052710",
    "한미반도체": "042700",
    "HPSP": "403870",
    "HD현대일렉트릭": "267260",
    ...
    "LS ELECTRIC": "010120",
    "두산에너빌리티": "034020",
    # 2026-09-08 추가 — ai_power_top5_picks T1의 「변압기 3~5년 vs 원전 8~15년」을 검증하려는데
    # 원전 쪽 4사 중 3사가 추적 밖이라 두산에너빌리티 하나로 대표하고 있었다(n=1).
    # 게다가 두산은 가스터빈 겸업이라 순수 원전 대리변수가 아니다 — xAI 가스터빈 뉴스로 오른 것을
    # 「원전이 올랐다」로 읽을 뻔했다. T1의 반증 조건을 재려면 이 셋이 있어야 한다.
    "오스코텍": "039200",   # §I-3 판정 이벤트 「3Q26 마일스톤 없는 분기」가 걸려 있는데 추적 밖이었다
    "비에이치아이": "083650",
    "한전KPS": "051600",
    "한전기술": "052690",
    "대한전선": "001440",
    ...
    "KX하이텍": "052900",
    "한화엔진": "082740",
    # STX엔진: 4행정 중속엔진 3사 중 마지막 미추적. HD현대중공업 발전엔진 증설
    # (2026-09-10 공시 8,336억·2028.5 완공)으로 「육상 발전엔진」 축이 열렸는데
    # 대조군이 한화엔진 하나뿐이라 섹터 판정이 1종목에 걸려 있었다.
    "STX엔진": "077970",
    "한국카본": "017960",
    "동성화인텍": "033500",
    "세진중공업": "075580",
    ...
    "현대차": "005380",
    "기아": "000270",
    # 2026-09-16 한투 「연료전지 — 블룸에너지 밸류체인」 5사 — 판단은 KIS TP가 아니라 우리 스크린(G/P/Q/E)으로 센다
    "비나텍": "126340", "한선엔지니어링": "452280", "아모센스": "357580", "서진시스템": "178320", "코세스": "089890",
    # 2026-09-20 추가 — 반도체 <소재> 층 16사. 왜: WATCH 195종목·스냅샷 225개인데 이 층이 0개였다.
    #   Citi 이세철 구간론(③ 2026-06-17)이 「전공정 → 후공정 → 소재(2027말~2028)」로 나누는데,
    #   09-18 원장으로 채점하니 전공정 6사/후공정 13사는 재고 소재는 <채점 자체가 불가>였다.
    #   구간이 실제로 와도 볼 장치가 없다는 뜻이라 지금 건다 — 도착률은 2027 부터 의미가 생긴다.
    # ⚠ 스냅샷에는 회사명 필드가 없어 <틀린 코드가 조용히 다른 회사를 받아온다>.
    #   그래서 등록 전에 네이버 basic 으로 stockName 을 전부 대조했다(러너 · 세션은 네이버 CONNECT 403).
    #   16/17 일치. 🔴 불일치 1건 — 155650 을 「오알켐」으로 적었는데 실제로는 <와이엠씨>였다.
    #   검증 안 했으면 트래커가 영원히 엉뚱한 회사를 오알켐이라 불렀을 것이다. 오알켐은 코드 미확인이라 뺀다.
    "솔브레인": "357780", "동진쎄미켐": "005290", "이녹스첨단소재": "272290", "원익머트리얼즈": "104830",
    # 2026-09-20 추가 — 국내 LNG 층 5사. 왜: 하나증권 LNG 노트(③ 09-20)의 Top Picks 4 중 3 이
    #   국내인데 WATCH 234종목·스냅샷 225개에 <하나도 없었다>. 잴 수 없는 것에 판단을 세우면
    #   ③ 이 ① 의 무게를 얻는다(§R1) — 그래서 그날은 판단을 세우지 않고 장치부터 걸었다.
    #   레짐(regime.one 09-15)이 「캘린더 밖 유가 충격(호르무즈)」을 이미 담고 있어 관측 가치가 있다.
    # ⚠ stockName 대조 5/5 일치(아침 소재 17건에서는 1건이 틀렸다 — 오알켐 → 와이엠씨).
    #   S-Oil 은 oil_hedge_vehicles T4 가 이미 다루는 종목이라 같이 건다(정유는 유가 헤지가 아니다).
    "포스코인터내셔널": "047050", "SK가스": "018670", "SK이노베이션": "096770",
    "한국가스공사": "036460",
    # ⚠ S-Oil(010950)은 이미 등록돼 있어 뺐다 — 2026-09-20 훅 검사가 잡았다.
    "후성": "093370", "한솔케미칼": "014680", "덕산네오룩스": "213420", "SKC": "011790",
    "엠케이전자": "033160", "램테크놀러지": "171010", "이엔에프테크놀로지": "102710", "디엔에프": "092070",
    "티씨케이": "064760", "하나머티리얼즈": "166090", "월덱스": "101160", "케이엔제이": "272110",
    # 2026-09-16 편입 규칙 v1(사용자 결정): 리포트·방송에서 매수/관심 콜로 등장한 종목은 WATCH(추적)로 자동 편입 — 판단은 스크린이, 페이지는 없다.
    #   출처: 삼프로 09-16(권태우 후공정·박지훈·하창완) · 신한 병목·건설 · 키움 SIB · 라이프자산운용 우선주 · KIS 연료전지 인접
    "SFA반도체": "036540",
    # 2026-09-20 추가 — 삼성 DDR5 모듈·SSD 외주화의 「모듈 조립(SMT)」 층. 아카이브는 이 외주화를 SFA반도체(칩 후공정) 한 곳에만 걸어 놨는데,
    #   외주로 나간 물건 자체의 순수 노출은 한양디지텍(삼성 97.1% · 모듈 82.1%+SSD 15% ②)이고 드림텍은 인도 신규 라인이다(brain T8).
    #   둘 다 이 세션에서 값을 못 재서(m.stock.naver.com 프록시 403) 정본 측정을 여기로 넘긴다.
    "한양디지텍": "078350",
    "드림텍": "192650",
    "유니테스트": "086390",
    "디아이": "003160",
...
WATCH_KR_ETF = {
    ...
    "TIGER 소프트웨어": "157490", "TIGER 헬스케어": "143860", "KODEX 반도체": "091160", "KODEX 미국AI광통신네트워크": "0173Y0", "SOL 조선TOP3플러스": "466920",
...
WATCH_US = {
    ...
    "Semtech": "SMTC.O",
    "Robinhood": "HOOD.O",  # 2026-09-16 페이퍼 신설
    "MACOM": "MTSI.O",
    "Fabrinet": "FN",
    ...
    "Salesforce": "CRM",
    "ServiceNow": "NOW",
    # 2026-09-20 추가 — 알리바바(BABA). Citi 압사라 2026 프리뷰(③)로 들어왔는데 추적도 판단도 0이었다.
    # 중국 AI 캐펙스 축(3년 3,800억 위안 · FY2031 외부 클라우드 $1,000억 · 2032 DC 10배)이
    # 아카이브에 「六張網」 서술(ai_value_chain_guide_v2 T5)로만 있고 회사 단위 값이 없다.
    # §B4 — 판단은 세우지 않고 시세만 받는다.
    "Alibaba": "BABA",
    # 2026-09-20 추가 — Natera(NTRA). 13F 상위 보유인데 아카이브 판단 0편이던 자리(brain ai_bio_investment_map T5).
    # Cerebras 는 2026-08-23에 이미 CBRS.O 로 들어와 있다(같은 dict 아래쪽) — 중복 키를 만들면 뒤엣것이 이긴다.
    "Natera": "NTRA.O",
    ...
    "NVIDIA": "NVDA.O",
    # 2026-09-14 사이버보안 편 카드 신설 — 6사 중 CRWD·PANW만 추적 중이었다.
    "Zscaler": "ZS.O", "Fortinet": "FTNT.O", "SentinelOne": "S", "Cloudflare": "NET",
    "AMD": "AMD.O",
    "Intel": "INTC.O",
    "ASML": "ASML.O",
    "SanDisk": "SNDK.O",
    ...
    "Broadcom": "AVGO.O",
    # ── 2026-09-01 추가. 🔴 「메모리 가격 → 칩 → 세트」 전가 사슬을 관측하려면
    # 사슬의 중간 마디가 필요하다. 퀄컴이 9/1 출하분부터 전 제품군 두 자릿수
    # 인상을 단행했고 CEO가 사유를 「DRAM·스토리지 가격 상승분을 그대로 전가」라
    # 밝혔다 — 전가를 실제로 하는 회사가 추적 밖이면 「멤플레이션이 세트 마진을
    # 깎는다」를 검증할 수 없다(§A7-0 — 사슬의 한 마디만 보면 어디서 깎이는지 모른다).
    # ⚠ 다만 퀄컴에는 애플 자체 모뎀 전환이라는 별개 변수가 섞여 있다(§A5).
    "Qualcomm": "QCOM.O",
    ...
    # ── 2026-09-08 추가 ──
    # 「판단은 있는데 시세가 없는」 공백을 세어 보니 8건이었고, 그중 상장사가 5곳이었다.
    # 전부 판정 이벤트가 걸려 있다 — 베라더믹스 남성 AGA 3상(2026-12) ·
    # 앱사이 1/2상 · 카인터 중국 3상 상태 변경 · 오스코텍 3Q26 마일스톤 없는 분기.
    # 어제 탈모 편을 만들며 값을 손으로 받았는데 추적에 안 넣어 다음에 또 손으로 받게 돼 있었다.
    # 2026-09-08 — 키옥시아는 티커 8종(6600.T·6600.JP·KIOXIA…)을 시도해 전부 409였다.
    # 원인은 형식이 아니라 <코드>였다 — 6600은 옛 도시바메모리 코드이고,
    # 키옥시아 홀딩스는 2024-12-18 도쿄 프라임 상장 시 <285A>를 받았다.
    # 「경로가 없다」가 아니라 「코드가 틀렸다」였고, 도요타(7203.T)로 대조해서야 갈렸다(§G).
    "Kioxia Holdings": "285A.T",
    "Veradermics": "MANE.K",
    "Absci": "ABSI.O",
    "Kintor Pharmaceutical": "9939.HK",
    "Cerebras": "CBRS.O",
    "Alphabet": "GOOGL.O",
    ...
    "Nextracker": "NXT.O",
    "Shoals": "SHLS.O",
    # 2026-09-16 편입 규칙 v1 — 클래리티 부결 직접 관련주(삼프로 크립토)
    "Coinbase": "COIN.O",
