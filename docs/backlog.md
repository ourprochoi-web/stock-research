...
## 배수를 믿어도 되는지 먼저 가른다 (2026-09-20 · `turn_screen.quality()`)

**「2Q 연율 PER」이 낮은 회사를 싸다고 읽으면 <한 분기가 튄 것>을 영속화한다.** 09-20 후공정 13사 실측:

| 종목 | 최근 5분기 OP | 단조 | CV | 2Q÷평균 | TTM PER | 2Q 연율 PER | 판정 |
|---|---|---|---|---|---|---|---|
| 이수페타시스 | 421→584→565→672→771 | — | **0.19** | 1.28 | 37.4 | 31.5 | 연율화 OK(안정) |
| **티엘비** | 69→87→86→107→128 | — | **0.21** | 1.34 | 27.1 | **20.5** | **연율화 OK(안정)** |
| **대덕전자** | 19→244→289→513→703 | **✓** | 0.66 | 1.99 | 33.5 | **22.1** | **연율화 OK(추세)** |
| 티에스이 | 148→115→198→420→525 | — | 0.58 | 1.87 | 23.2 | 15.8 | ⚠ 변동 |
| 디아이 | 123→100→43→189→**396** | — | 0.72 | **2.33** | 20.1 | **7.0** | 🔴 연율화 금지 |
| 한미반도체 | 863→678→276→85→**1303** | — | 0.67 | 2.03 | 100.9 | 51.7 | 🔴 톱니 — 무의미 |
| 심텍 | 55→124→103→137→**629** | — | **1.01** | **3.00** | 적자 | 26.6 | 🔴 연율화 금지 |

**규칙** — `annualizable(mono, cv)`: **단조 추세이거나 CV ≤ 0.25 일 때만** 연율 배수를 값으로 쓴다.
· 단조 ✓ + CV 높음 = **추세**(낮은 데서 시작해 계속 올랐다) → OK
· 단조 ✗ + CV 높음 = **변동**(톱니) → 금지
**「싸 보이는 집합」과 「이익의 결이 좋은 집합」은 겹치지 않는다** — 디아이 7.0배는 변동의 산물이고, 티엘비 20.5배가 값이다.

⚠ 손실 분기도 같은 문제를 반대로 만든다: 심텍 TTM PER 이 −105인 이유는 4Q25 순손실 −1,168억 하나다.
**TTM 배수를 쓰기 전에 손실 분기가 물려 있는지 본다.**
