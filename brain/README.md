...
| `entities.json` | 회사·테마 카드 — 무슨 사업(등급 표기)·층·정본 페이지·facts 키·걸린 테제 · **`watch[]`(2026-09-26)** = 테제에 안 걸린 관측의 종합 `{date, grade, rid, one, source?}` · `status:"후보"` = 페이지 없는 회사(watch 만) | 회사를 새로 다룰 때 · **테제 없는 회사 관측이 올 때**(버리지 않는다) |
...
- **`entity:<KEY>`(2026-09-26)** — 테제에 안 걸리지만 회사가 보이는 관측. `entities.companies[KEY].watch[]` 에 `rid == id` 인 줄이 있어야 통과(테제 접촉과 같은 원리). 왜: 「판정」만 쌓이고 「이 회사에 무엇이 쌓였나」가 없어 결정 시점의 그림이 비었다 — `packet.py` 가 저울(ⓐ/ⓑ·등급) · 지난 번 이후 · 반대편 · watch · 비어 있는 칸으로 그 그림을 준다.
