...
def render_market(rg):
    ...
    return (f"{MSTART}\n"
        ...
        f'<div><b>금리</b> — {e(m["rates"]["state"])}<br><span style="{SUB}">10Y {m["rates"]["us10y"]}% · 2Y {m["rates"]["us2y"]}% · 30Y {m["rates"]["us30y"]}% · 다음 {e(m["rates"]["next"])}</span></div>'
        f'<div><b>유가</b> — {e(m["oil"]["state"])}<br><span style="{SUB}">브렌트 {m["oil"]["brent"]} ({e(m["oil"]["asof"])}) · 프리미엄 {e(m["oil"]["premium_est"])}<br>깨지면: {e(m["oil"]["breaks_if"])}</span></div>'
        f'<div><b>가스</b> — {e(m["gas"]["state"])}<br><span style="{SUB}">HH {m["gas"]["hh"]} · TTF {e(m["gas"]["ttf"])} · JKM {e(m["gas"]["jkm"])} · 전환 신호 {e(m["gas"]["transition_signal"])}</span></div>'
        f'<div><b>한국</b> — {e(k["kr"]["phase"])}<br><span style="{SUB}">코스피 {k["kr"]["kospi"]} · 고점({e(k["kr"]["peak"]["date"])}) 대비 {k["kr"]["drawdown_pct"]}% · {e(k["kr"]["breadth"])} · {e(k["kr"]["valuation_proxy"])}</span></div>'
        f'<div><b>미국</b> — {e(k["us"]["phase"])}<br><span style="{SUB}">S&P {k["us"]["sp500"]} · VIX {k["us"]["vix"]} · {e(k["us"]["read"])}</span></div>'
        f'<div><b>수급</b> — 외국인 {e(fl["kr_foreign"]["net"])}<br><span style="{SUB}">{e(fl["kr_foreign"]["read"])} · 개인 {e(fl["kr_individual"]["net"])} · 환율 {m["fx"]["usdkrw"]}</span></div>'
        f'</div>\n'
